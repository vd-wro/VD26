...
const int BLUE_RED_THRESHOLD =   70;   // blue color thresholds. og = 80
...
const int ORANGE_BLUE_THRESHOLD =  62;
...
const long correctionCooldownMillis = 8000; // time between each correction
...
void avoidWallPID() {
  ...
  if (!turningInProgress && lapTurnCount != 0) {
    ...
    // PID lateral
    float error = distance - distanceToWall;  // queremos mantener 20 cm de distancia
...
void avoidWall(int correctionAmount) {
  ...
  if (!turningInProgress && !correctionState && lapTurnCount != 0) {
    ...
    if(distance < 15 && distance != 0 && correctionFarCount < 2) { // if it is too close to he wall
