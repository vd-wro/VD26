...
int turnAngle = 90; // Defining a variable to compensate for any constant offset in every turn
...
// PID constants
const float Kp = 2.3; // proportional gain
const float Kd = 1.7; // derivative gain
...
const int BLUE_BLUE_THRESHOLD =  80;
...
const int BLUE_RED_THRESHOLD =   80;   // blue color thresholds. og = 80
const int BLUE_GREEN_THRESHOLD = 86;
const int BLUE_BLUE_THRESHOLD =  73;
...
// Signatures for the OpenMV color recognition
#define SIGNATURE_RED 2 //2
#define SIGNATURE_GREEN 1 //1
 
/*Configuration of the Region of Interest
Coordinates of the OpenMV's resolution: width: 160, height: 120*/
...
const int ROI_Y_BOUND = 119/6;
 
const int IMAGE_WIDTH = 159;
...
void initHeadlights(int ledBrightness) {
...
void encoderDelayOrientation(unsigned long cm) {
  ...
  int pulsesRequired = PULSES_PER_CM * 5 * cm; // Calculate required pulses
  ...
  while (1) {
    ...
    if (((pulsesRequired - encoderPulseCount) <= 0)) {
      break;
    }
  }
...
void parkingManeuver() {
  ...
  if (parkingManeuverBlockDetection()) {
    setSteeringAngle(SERVO_STRAIGHT);
    digitalWrite(ledPin, HIGH);
    delay(1000);
    driveBackward(parkingSpeed);
    delay(4000);
    ...
    if (direction == +1) {
      ...
      driveForward(parkingSpeed);
        ...
        encoderDelayOrientation(22); // Waits until the defined distance is complete
        stopBrake();
        setSteeringAngle(SERVO_STRAIGHT); // Resets Servo Angle
        delay(250);
        driveBackward(parkingSpeed);            
        encoderDelay(63); // Waits until the defined distance is complete
      ...
      driveForward(parkingSpeed);
        ...
        encoderDelayOrientation(45); // Waits until the defined distance is complete
        stopBrake();  // Stops the motors
        ...
        distance = getDistance(sonarLeft);
        a_cm = distance - 52;
        b_cm = 42;
        c_cm;
        angle;
        ...
        driveBackward(parkingReverseSpeed);
        encoderDelay(29); // Waits until the defined distance is complete
        delay(250);
  ...
  if (direction == -1) {
    ...
    setSteeringAngle(SERVO_LEFT);
    driveBackward(parkingReverseSpeed);
    encoderDelay(31); // Waits until the defined distance is complete
        delay(250);
...
bool parkingManeuverBlockDetection() {
  ...
  int blockSignature = pixy.ccc.blocks[0].m_signature;
...
if (distance == 0) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (distance < 15) {  // if the distance to the wall is short,
      ...
      turnTargetYaw = (turnTargetYaw + ((turnAngle + correction) * -direction));  // sets the target yaw to ± 90°, according to direction
      setTargetYaw(turnTargetYaw);
      ...
      if(lapTurnCount == 4 || lapTurnCount == 8 || lapTurnCount == 12) {
        ...
        stopBrake();
        safeDelay(500);
        turnTargetYaw = (turnTargetYaw * -direction);  // sets the target yaw to ± 90°, according to direction
        setTargetYaw(turnTargetYaw);
        return;
      } else {
        stopBrake();
...
void handleEvasion() {
  ...
  while (true) {
    readBlock(); // Get the latest block data in every iteration.
    updateOrientation();
    if (detectFloorColor()) {
      handleColorAction();
      break;
    ...
    // 3. If a relevant block is found, decide the action.
      ...
      int currentSignature = currentColor;
      ...
      if (activeSignature == 0) {
        ...
        if (activeSignature == SIGNATURE_RED) {
          ...
          // if (lapTurnCount == lapFinishedNumber && direction == +1)  return; // TAPE????
      ...
      /*
      // EMERGENCY EXIT (Optional function in case the camera loses the block or something goes wrong with the evasion, to avoid getting stuck in the loop):


      if (yaw > ((direction * 130) + targetYaw)) {
        int angle = (direction < 0) ? SERVO_RIGHT+10 : SERVO_LEFT-10; // sets the servo angle according to the direction
        stopBrake();
        safeDelay(250);
        steeringServo.write(angle); // sets the servo to the modified sharper angle
        safeDelay(150);
        driveBackward(backwardSpeed); // drive backwards until the robot corrects its orientation
        while (yaw > targetYaw + (direction * 85)) {
          updateOrientation();
        }
        stopBrake();
        safeDelay(100);
        ...
        if (df < 30) {
          ...
          while (df < 30 || df == 0) {  // while the distance to the wall is greater than 10
            df = getDistance(sonarFront); // updates the front distance
            updateOrientation();  // keeps the orientation
            safeDelay(10);  // brief delay
