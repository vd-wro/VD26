...
Functions like AWB (auto-white-balance), auto-gain, and auto-exposure are disabled to maintain stable lighting during the whole trajectory. Start-up AWB gain overwrite and auto-gain coeficients are configured to a specific value, attempting to achieve similar conditions every time the camera boots; it is not possible to initialize the camera without a start-up AWB, gain, and exposure. For this white balance configuration specifically, red and blue gains are incremented, it balances out the initial start-up AWB. This balance is required since the robot starts with the magenta wall in front, and it is necesarry to boost these values to achieve a clean white balance.
